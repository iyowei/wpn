...
sc_send() {
    ...
    # 如果没有提供密钥，尝试从配置文件读取
    if [ -z "$key" ]; then
        local config_file="$(dirname "${BASH_SOURCE[0]}")/../.env"
    ...
    local encoded_desp=$(url_encode "$desp")
    ...
    # 使用动态生成的 url 发送请求
    local result=$(curl -X POST -s -o /dev/null -w "%{http_code}" "$url" "${opts[@]}" 2>/dev/null)
...
check_notification_config() {
    ...
    local sendkey=$(grep '^SENDKEY=' "$config_file" | cut -d'=' -f2 | tr -d '"'"'"'')
